...
### 07. Web Services - дефиниция, възможности и реализации. Описание на SOAP, WSDL, UDDI.

Уеб услугите са набор от стандартни протоколи и технологии за обмен на данни между приложения или системи. Тези технологии включват XML, SOAP, REST, JSON, WSDL and UDDI и позволяват на системите да комуникират и споделят данни през мрежата чрез стандартизирана схема, която кодира комуникацията между отделните системи. По този начин уеб услигите осигуряват стандартизиран подход за изграждане на разпределени системи и архитектурата, ориентирана към услуги (SOA). 

Има много технологии, протоколи и стандарти, включени в създаването и използването на уеб услуги. Понякога може да бъде малко предизвикателство да ги класифицираме и категоризираме, тъй като те често се припокриват и могат да се използват в комбинация един с друг. И все пак ето обща разбивка:

#### 1. Протоколи
- **SOAP** е протокол за съобщения, използван за размяна на структурирана информация между клиентите и сървърите. Той използва XML-базиран формат и транспортните протоколи HTTP и SMTP.
SOAP съобщенията се състоят от три главни части:
    - envelope - обвивка, която определя структурата на съобщението и как то да бъде обработено.
    - header (съдържат допълнителна информация за съобщението)
    - body (съдържа реалните данни, които се предават), което позволява разширяемост и включване на допълнителни метаданни.
- **REST** е архитектурен стил за мрежови хипермедийни приложения. Често се използва при разработването на уеб услуги и въпреки че не налага използването на JSON, често се свързва с него. REST протоколът е stateless, което означава, че всяка заявка от клиент към сървъра съдържа цялата необходима информация за сървъра, за да разбере и обработи заявката. 
Стандартните операции в REST са базирани на HTTP методи и обикновено включват:
    - GET: Извлича конкретен ресурс (по id) или колекция от ресурси.
    - POST: Създаване на нов.
    - PUT: Изцяло заменя съществуващ ресурс.
    - PATCH: Частично актуализира съществуващ ресурс.
    - DELETE: Премахване на определен ресурс.
    - HEAD: Извлича head-ърите на ресурса, без да изтегля цялото му съдържание. Използва за проверка на статуса или свойствата на ресурса.
    - OPTIONS: Извлича разрешените методи и възможности на ресурса. Полезен е за определяне на поддържаните операции върху даден ресурс. Обикновенно е служебен метод, но може да се ползва и ръчно.

- **WebSocket** е комуникационен протокол, който прави възможно установяването на двупосочен комуникационен канал между сървър и клиент.
- **RPC (Remote Procedure Call):** е протокол, който една програма може да използва, за да поиска услуга от програма, разположена в друг компютър в мрежа. Основната идея на RPC е да скрие подробностите за мрежовата комуникация от програмиста. Тя позволява на разработчиците да създават приложения, сякаш всички са на една и съща машина, вместо да са разпръснати в мрежа. Пример за модерна, високопроизводителна RPC рамка е gRPC на Google. 

#### 2. Дефиниций на уеб услуги
- **WSDL** е XML-базиран език за описание на интерфейса и възможностите на уеб услуга. Той предоставя машинно-четимо описание на операциите, входните и изходните параметри и поддържаните формати на съобщения от услугата. WSDL позволява на клиентите да откриват и разбират как да взаимодействат програмно с уеб услуга.
- **OpenAPI (formerly Swagger)** е спецификация за описание на машинно-четими файлове интерфейси, за описване, създаване, ползване и визуализиране на RESTful уеб услуги. Swagger файловете могат да бъдат написани на YAML или JSON синтакс и също така могат да се ползват различни инструменти за генериране на клиенти, сървъри и документация за тези интерфейси.

#### 3. Регистър за уеб услуги.
- **UDDI (Universal Description, Discovery, and Integration)** е директория/глобален регистър, която облекчава откриването и регистрацията на уеб услуги. Той осигурява стандартизиран подход за публикуване, откриване и извикване на услуги между различни организации и платформи. UDDI използва йерархична структура за категоризиране на услугите и позволява на бизнесите да рекламират услугите си по стандартизиран начин.

#### Мултимедийни приложение в web. Web графика. Flash. Аудио формати за обработка. Видео формати. Streaming. 
Мултимедийните приложения в мрежата включват интерактивни уебсайтове (google maps, scaleofuniverse.com), браузър-базирани онлайн игри, музикални платформи, видео платформи (netflix, hbo, disney) др. Те предлагат по-богато и по-ангажиращо потребителско изживяване чрез интегриране на текст, графики, анимации, видео и звук.

По дефиниция мултимедията е представяне на информация по привлекателен и интерактивен начин с помощта на комбинация от текст, аудио, видео, графики и анимация.

##### Web графика.
Уебсайтовете и приложенията често трябва да представят най-различни графики. Графиката която уеб поддържа е както статична, така и 2D, 3D и видео графика.

Най-често ползваните файлови формати за статична уеб графика, са:
- JPEG (Joint Photographic Experts Group) – Този формат се използва най-добре за фотографски изображения или изображения с много цветове.
- PNG (преносима мрежова графика) - PNG е подходящ за сложни изображения, като лога, които изискват прозрачност и високи нива на детайлност.
- SVG (Scalable Vector Graphics) - SVG се използва за векторно базирани диаграми, като карти и лога. SVG графиките могат да се мащабират до различни размери, без да губят качество, което ги прави много полезни за адаптивен уеб дизайн.
- GIF (Graphics Interchange Format) – GIF се използват за малки, прости графики и за анимации.

2D, 3D, сложни анимаций и видео/аудио
- Canvas предоставя средство за рисуване на 2D графики чрез JavaScript посредством HTML canvas елемента.
- WebGL (Web Graphics Library) е JavaScript API за изобразяване на високопроизводителни интерактивни 3D и 2D графики във всеки съвместим уеб браузър без използването на добавки.
- HTML audio и video елементите позволяват вграждане на видео и/или аудио в уеб страница и контролиране на нейното възпроизвеждане.

#### Flash
Flash беше популярна технология за уеб базирани мултимедийни приложения, включително анимации, видео и аудио. Разработено от Adobe Systems, то позволява богато, интерактивно преживяване в мрежата. Той обаче беше критикуван за проблемите си със сигурността и голямото потребление на ресурси и вече не се поддържа от края на 2020 г. Повечето съвременни уеб технологии преминаха към HTML5, който предлага вградена поддръжка за видео и аудио елементи, и Canvas или WebGL за сложни анимации и игри.

##### Аудио формати за обработка
Има много налични аудио формати, но най-често срещаните, които ще видите, са WAV, MP3 и AAC.
- **WAV (формат на аудио файл с вълнова форма)** - WAV е аудио формат без загуба, което означава, че не губи никакви данни в процеса на компресиране. Осигурява висококачествен звук, но заема повече място за съхранение.
- **MP3 (MPEG Audio Layer III)** - Това е общ аудио формат, известен със способността си да компресира файлове без значителна загуба на качество.
- **AAC (Advanced Audio Coding)** - AAC е аудио формат със загуба, който осигурява по-добро качество на звука от MP3 при подобни битрейтове.

##### Видео формати
Често срещаните видео формати включват MP4, AVI, MOV и FLV.
- **MP4 (MPEG-4 част 14)** - MP4 е цифров мултимедиен формат, който най-често се използва за съхраняване на видео и аудио. Известен е със своите възможности за високо качество и компресия.
- **AVI (Audio Video Interleave)** - AVI е формат на мултимедиен контейнер, въведен от Microsoft. Той е по-малко компресиран от формати като MP4, което води до по-големи размери на файловете.
- **MOV (QuickTime Movie)** - MOV е мултимедиен формат, разработен от Apple. Обикновено се използва за запазване на филмови и видео файлове с висококачествен изход.
- **FLV (Flash Video Format)** - FLV беше популярен избор за вградено видео в уебсайтове поради способността му да се възпроизвежда във Flash player. Той обаче до голяма степен е заменен от формати като MP4.

##### Streaming
Streaming е процес по поточно предаване на данни или получаване на данни (особено видео и аудио материал) през компютърна мрежа като постоянен, непрекъснат поток, позволяващ възпроизвеждането да продължи, докато се получават последващи данни. Той се превърна в предпочитан метод за доставка на видео и аудио в мрежата, използван от платформи като Netflix, YouTube и др.

Технологията зад стрийминг се разви през годините, като първоначално се ползвал RTMP (Real-Time Messaging Protocol), а днес се ползват HTTP-базираните стрийминг протоколи, който стават все по-често срещани, като HLS (HTTP Live Streaming) и MPEG-DASH (Dynamic Адаптивно поточно предаване през HTTP).